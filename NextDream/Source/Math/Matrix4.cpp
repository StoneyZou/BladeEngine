...
namespace dream
{
	...
	Matrix4 Matrix4::SetRotationByAxis( const Vector3& axis, f32 radiens )
	{
		...
		f32 sinR = Math::Sin( radiens );
		f32 cosR = Math::Cos( radiens );
	...
	EulerAngles Matrix4::Matrix4ToEulerAngles() const
	{
		...
		if(Math::Abs(sp) > 9.99999f )
		{
			...
			temp.heading_ = Math::ATan2( -mM[1][2], mM[0][0] );
			temp.bank_ = 0.0f;
		}
		else
		{
			temp.heading_ = Math::ATan2( mM[2][0], mM[2][2] );
			temp.pitch_ = asin( sp );
			temp.bank_ = Math::ATan2( mM[0][1], mM[1][1] );
		}
	...
	Quaternion Matrix4::Matrix4ToQuaternion() const
	{
		...
		if( diag > 0.0f )
		{
			const f32 scale = Math::Sqrt(diag) * 2.0f; // get scale from diagonal
		...
		else
		{
			if ( mM[0][0] > mM[1][1] && mM[0][0] > mM[2][2] )
			{
				...
				const f32 scale = Math::Sqrt( 1.0f + mM[0][0] - mM[1][1] - mM[2][2] ) * 2.0f;
			...
			else if ( mM[1][1] > mM[2][2] )
			{
				...
				const f32 scale = Math::Sqrt( 1.0f + mM[1][1] - mM[0][0] - mM[2][2] ) * 2.0f;
