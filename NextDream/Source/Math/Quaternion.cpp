...
namespace dream
{
	...
	void Quaternion::SetToRotateAxis(const Vector3& axis, f32 theta)
	{
		...
		f32 sinThetaOver2 = Math::Sin(thetaOver2);

		mW = Math::Cos(thetaOver2);
	...
	Quaternion Slerp(const Quaternion& q0, const Quaternion& q1, f32 t)
	{
		...
		else
		{
			//平滑插值
			//计算两四元数间夹角的sin
			f32 sinOmage = Math::Sqrt(1.0f - cosOmega * cosOmega);

			//通过sin值和cos值计算角度
			f32 omage = Math::ATan2(sinOmage, cosOmega);

			//计算分母的倒数
			f32 oneOverSinOmage = 1.0f / sinOmage;
			
			//计算插值
			k0 = Math::Sin((1.0f - t) * omage) * oneOverSinOmage;
			k1 = Math::Sin(t * omage) * oneOverSinOmage;
		}
	...
	EulerAngles Quaternion::QuaternionToEularAngles() const
	{
		...
		//检查万向锁
		if(Math::Abs(sp) > 0.999f)
		{
			temp.pitch_ = Math::HALF_PI_F * sp;

			//发生万向锁,把bank旋转量转移到heading
			temp.bank_ = 0.0f;
			temp.heading_ = Math::ATan2(-mX*mZ - mW*mY, 0.5f - mY*mY - mZ*mZ);
		}
		else
		{
			temp.pitch_ = Math::ASin(sp);
			temp.heading_ = Math::ATan2(mX*mZ - mW*mY, 0.5f - mX*mX - mY*mY);
			temp.bank_ = Math::ATan2(mX*mY - mW*mZ, 0.5f - mX*mX - mZ*mZ);
		}
